...
class _RemotePageState extends State<_RemotePage> {
  ...
  // Тот же поворот, посчитанный по системной ориентации: эталон без дрейфа,
  // к которому подтягивается _phoneYaw. Отсчёт общий - оба начинаются с нуля.
  ...
  // Тот же поворот, но по АБСОЛЮТНОЙ ориентации от системы. Она слита с
  // магнитометром (Android TYPE_ROTATION_VECTOR, iOS - рамка с коррекцией),
  // поэтому у неё есть опора и она не уползает - в отличие от гироскопа,
  // которому опереться не на что.
  //
  // Это НЕ источник поворота, а только эталон, к которому медленно
  // подтягивается посчитанный гироскопом угол (см. _feedGyro). Напрямую брать
  // её нельзя: магнитометр время от времени перекалибровывается, и ориентация
  // скачком уезжает на несколько градусов. Гироскоп таких скачков не даёт,
  // поэтому за движением следит он, а компас лишь убирает накопленный дрейф.
  //
  ...
  void _feedAttitude(double w, double x, double y, double z) {
    if (_haveAttitude) {
      ...
      if (dYaw.abs() < 45) {   // разумный шаг; больше - сбой потока, пропускаем
        _compassYaw += dYaw;
        if (_compassYaw > 180) _compassYaw -= 360;
        if (_compassYaw < -180) _compassYaw += 360;
      }
    }
  ...
  void _feedGyro(double rawX, double rawY, double rawZ) {
    ...
    if (_phoneTilt == 2) {
      // За движением следит гироскоп: он гладкий и не даёт скачков. Проекция
      // вектора вращения на направление силы тяжести даёт скорость поворота
      // вокруг вертикали мира, независимо от того, как держат телефон.
      ...
      // практически неподвижен и очень медленно. Причина: системная ориентация
      // время от времени скачком перекалибровывается по магнитометру, и если
      // тянуться к ней быстро, скачок уедет прямо в мотор. Пока телефон ведут,
      // поправка не нужна и только мешала бы - там всё решает гироскоп, а
      // расхождение между датчиками по времени как раз максимально.
      if (_useCompass && _haveAttitude && rateRaw.abs() < 0.02) {  // ~1 °/с
        var err = _compassYaw - _phoneYaw;
        if (err > 180) err -= 360;
        if (err < -180) err += 360;
          ...
          if (_source == RemoteSource.phone) ...[
            ...
            SwitchListTile(
              ...
              onChanged: (v) {
                setState(() {
                  _useCompass = v;
                  // Включаем - начинаем эталон от текущего угла, иначе
                  // накопленное за время выключения расхождение поехало бы
                  // выбираться поправкой.
